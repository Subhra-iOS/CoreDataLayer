//
//  SRCoreDataMigrationManager.swift
//  SRCoreDataLayer
//
 ...
 */
class SRCoreDataMigrationManager {
    
    // MARK: - Check
    
    func requiresMigration(at storeURL: URL, currentMigrationModel: SRCoreDataMigrationModel = SRCoreDataMigrationModel.current) -> Bool {
    ...
    func migrateStore(at storeURL: URL) {
        migrateStore(from: storeURL, to: storeURL, targetVersion: SRCoreDataMigrationModel.current)
    }
    
    func migrateStore(from sourceURL: URL, to targetURL: URL, targetVersion: SRCoreDataMigrationModel) {
    ...
    func forceWALCheckpointingForStore(at storeURL: URL) {
        guard let metadata = NSPersistentStoreCoordinator.metadata(at: storeURL), let migrationModel = SRCoreDataMigrationModel.migrationModelCompatibleWithStoreMetadata(metadata)  else {
            return
        }
